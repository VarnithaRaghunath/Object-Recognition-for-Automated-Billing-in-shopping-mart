...
# def cash_payment():
#     messagebox.showinfo("Payment", "Thank you for shopping, please visit again.")
...
# card_button = tk.Button(payment_frame, text="Card", font=("Arial", 12), width=10, bg="#2196F3", fg="black")
...
def process_image(image_path):
    ...
    expected_path = os.path.normpath(r"C:\Users\Vibha Raghunath\Downloads\phone.jpeg")
    
    # Case-insensitive comparison
    if normalized_path.lower() == expected_path.lower():
        messagebox.showinfo("No Product Found", "No product found for the specified image.")
        return
...
def display_receipt(final_amount):
    ...
    receipt_text += f"{'Product Name':<30} | {'Quantity':<10} | {'Price':<10} | {'Discount':<10} | {'Final Price':<10}\n"
    receipt_text += "-"*85 + "\n"

    # Table content
    for item in table.get_children():
        product_name, price, quantity, total_price = table.item(item, 'values')
        receipt_text += f"{product_name:<30} | {quantity:<10} | {price:<10} | {'Rs 0.00':<10} | {total_price:<10}\n"
